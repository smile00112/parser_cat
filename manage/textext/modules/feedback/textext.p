...
################################################################################
@auto[]
$self.classVersion[1.81]
$self.classBuildDate[30.11.2015]
...
################################################################################
@GetBlocks[]
$result[^table::sql{SELECT * FROM $self.blocksTable.name WHERE block=1 ORDER BY id}]
################################################################################
@GetModuleIdByName[name]
$result(^int:sql{SELECT id FROM $self.blocksTable.name WHERE nameblock='$name'}[$.default[$name]])
################################################################################
@GetModulePathByName[name]
$result[^string:sql{SELECT pathblock FROM $self.blocksTable.name WHERE nameblock='$name'}[$.default[$name]]]
################################################################################
@GetBlockDescriptionByName[name]
$result[^string:sql{SELECT namemenu FROM $self.blocksTable.name WHERE nameblock='$name'}[$.default[$name]]]
################################################################################
@GetPageBlocks[id]
$result[^table::sql{SELECT id, name, pref_block, sortir FROM $self.pageBlocksTable.name WHERE idpage=$id ORDER BY sortir}]
################################################################################
...
$result(^void:sql{UPDATE $site:menusTable.name SET $property='$value' WHERE id=$id})
################################################################################
...
# Блоки в одном экземпляре
   ^if($pageBlocks.pref_block eq 'news' || $pageBlocks.pref_block eq 'catalog' || $pageBlocks.pref_block eq 'comments' || $pageBlocks.pref_block eq 'responses'){
    ^if(^self.GetCountPageBlock[$idpage;$pageBlocks.pref_block]>0){$text[${text}$pageBlocks.pref_block^#0A]}
   }
  }
# Если есть блок "redirect"
 }{$text[${text}$pageBlocks.pref_block^#0A]}
}
...
################################################################################
@GetNonPagedBlocks[]
$result[^table::sql{SELECT * FROM $self.pageBlocksTable.name WHERE idpage=0}]
################################################################################
@GetOptions[blockId]
...
$flag(0)
^blocks.menu{
 ^if($blocks.pref_block eq '$blockType'){$flag(1)}
}
$result($flag)
################################################################################
...
################################################################################
@GetCmsSections[level]
$result[^table::sql{SELECT id, namemenu, nameblock, pathblock FROM $self.blocksTable.name WHERE block<>1 AND level<=$level ORDER BY id}]
################################################################################
@UpdateModulePathByName[name;path]
^try{
	^void:sql{UPDATE $self.blocksTable.name SET pathblock="$path" WHERE nameblock="$name"}
	$result(1)
}{
	$exception.handled(true)
	$result(0)
...
^if(def $params.moduleName){
	...
	^try{
		...
		$counter(0)
		^use[${params.moduleName}_cms.p;$.replace(true)]
		^process{^$this[^^${params.moduleName}_cms::create[]]}
