...
^if(def $this.users){
	^this.users.good.menu{
		^if(!^this.GetCmsUserByLogin[$this.users.good.login]){<tr><td>Пользователь <font color="red"><strong>$this.users.good.name ($this.users.good.login)</strong></font> не зарегистрирован в системе.</td><td><a href="">Установить</a></td></tr>^errors.inc[]}
	}
	^this.users.bad.menu{
		^if(^this.GetCmsUserByLogin[$this.users.bad.login]){<tr><td>Пользователь <font color="red"><strong>$this.users.bad.name</strong></font> зарегистрирован в системе.</td><td><a href="">Удалить</a></td></tr>^errors.inc[]}
	}
^if($errors<1){<tr><td colspan="2"><font color="green"><strong>Проблем с пользователями не найдено</strong></font></td></tr>}
}{
	<tr><td colspan="2"><div class="cms_h1">Не удалось получить список пользователей</div></td></tr>
}
</table>
################################################################################
@IntegrityCheck[]
...
@SeoAttributesCheck[]
<hr>
$contentModule[^modules:GetModules[$.name[content]]]
...
^if(def $clean_attributes){
 ...
 <table border="0" cellspacing="1" cellpadding="5" bgcolor="#999999">
  ...
  <tr class="tN">
   <td>  
    <a href="/manage/$contentModule.path/?contentid=$clean_attributes.id" alt="Перейти в наполнение" title="Перейти в наполнение">$clean_attributes.name</a>
   </td>
   <td>
...
@UrlCheck[]
<hr>
$contentModule[^modules:GetModules[$.name[structure]]]
...
^if(def $clean_uri){
 ...
 <table border="0" cellspacing="1" cellpadding="5" bgcolor="#999999">
  ...
  <tr class="tN">
   <td>  
    <a href="/manage/$contentModule.path/#zap$clean_uri.id" alt="Перейти к странице" title="Перейти к странице">$clean_uri.name</a>
   </td>
   <td>
    $clean_uri.parent
   </td>
